...
TESTCASE(tivxLaplacianPyramid, CT_VXContext, ct_setup_vx_context, 0)

#define LEVELS_COUNT_MAX    7

TEST(tivxLaplacianPyramid, testNegativeNodeCreation)
{
...
static void own_negative_laplacian_pyramid_openvx(vx_context context, vx_border_t border, vx_image input, vx_pyramid laplacian, vx_image output)
{
    vx_graph graph = 0;
    vx_node node = 0;
...
static void own_negative_laplacian_reconstruct_openvx(vx_context context, vx_border_t border, vx_pyramid laplacian, vx_image input, vx_image output)
{
    vx_graph graph = 0;
    vx_node node = 0;
...
TEST_WITH_ARG(tivxLaplacianReconstruct, negativeTestBorderMode, Arg, NEGATIVE_LAPLACIAN_RECONSTRUCT_PARAMETERS)
{
    vx_uint32 i;
    vx_context context = context_->vx_context_;
    vx_size levels = 0;
    ...
    vx_border_t border = arg_->border;
    vx_border_t build_border = arg_->border;

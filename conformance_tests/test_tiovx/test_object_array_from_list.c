...
TESTCASE(ObjectArrayFromList, CT_VXContext, ct_setup_vx_context, 0)
...
vx_enum IMG_FORMATS[3] = {
    (vx_enum)VX_DF_IMAGE_U8,
    (vx_enum)VX_DF_IMAGE_RGB,
...
static vx_reference own_create_reference(vx_context context, vx_enum item_type, vx_int8 variance)
{
    ...
    vx_int32 offset = 0;
    vx_uint32 range = 360;
    vx_enum thresh_types[2] = {VX_THRESHOLD_TYPE_BINARY, VX_THRESHOLD_TYPE_RANGE};
    ...
    switch (item_type)
    {
        ...
        case VX_TYPE_PYRAMID:
            ...
            break;
        case VX_TYPE_SCALAR:
            exemplar = (vx_reference)vxCreateScalar(context, obj_item_type, &(values[variance % 3]));
            break;
        case VX_TYPE_MATRIX:
            exemplar = (vx_reference)vxCreateMatrix(context, obj_item_type, m + variance, n + variance);
            break;
...
static void own_check_meta(vx_reference item, vx_reference ref)
{
    ...
    switch (item_type)
    {
        ...
        case VX_TYPE_TENSOR:
        {
            ...
            VX_CALL(vxQueryTensor((vx_tensor)ref, VX_TENSOR_NUMBER_OF_DIMS, &ref_num_dims, sizeof(vx_size)));
            VX_CALL(vxQueryTensor((vx_tensor)item, VX_TENSOR_NUMBER_OF_DIMS, &item_num_dims, sizeof(vx_size)));
            ASSERT(item_num_dims == ref_num_dims);

            VX_CALL(vxQueryTensor((vx_tensor)ref, VX_TENSOR_FIXED_POINT_POSITION, &ref_fixed, sizeof(vx_int8)));
...
TEST_WITH_ARG(ObjectArrayFromList, testTIVXCreateObjectArray, Obj_Array_Arg, PARAMETERS)
{
    ...
    vx_bool isFromList = vx_false_e;

    vx_reference actual_items[OBJECT_ARRAY_NUM_ITEMS] = {0};

    vx_reference test_item;

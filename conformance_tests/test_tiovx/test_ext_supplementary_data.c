...
static vx_status VX_CALLBACK  myUserTestTargetFunction(tivx_target_kernel_instance kernel, tivx_obj_desc_t *obj_desc[], uint16_t num_params, void *priv_arg)
{
    ...
    */
    ...
    ERROR_EXPECT_STATUS(tivxSetSupplementaryDataObjDesc(obj_desc[3], NULL), VX_ERROR_INVALID_REFERENCE, "tivxSetSupplementaryDataObjDesc returns VX_ERROR_INVALID_REFERENCE when passed NULL source");
    ERROR_EXPECT_STATUS(tivxSetSupplementaryDataObjDesc(NULL, supp), VX_ERROR_INVALID_REFERENCE, "tivxSetSupplementaryDataObjDesc returns VX_ERROR_INVALID_REFERENCE when passed NULL destination");
    ERROR_EXPECT_STATUS(tivxSetSupplementaryDataObjDesc(obj_desc[1], supp), VX_FAILURE, "tivxSetSupplementaryDataObjDesc returns VX_FAILURE when destination has no supplementary data even though parent has");
    ERROR_EXPECT_STATUS(tivxSetSupplementaryDataObjDesc(obj_desc[2], supp), VX_FAILURE, "tivxSetSupplementaryDataObjDesc returns VX_FAILURE when destination has no supplementary data");
    ERROR_EXPECT_STATUS(tivxSetSupplementaryDataObjDesc(obj_desc[3], supp), VX_SUCCESS, "tivxSetSupplementaryDataObjDesc returns VX_SUCCESS with correct parameters");
    supp->type_name[0]++;
    ERROR_EXPECT_STATUS(tivxSetSupplementaryDataObjDesc(obj_desc[3], supp), VX_ERROR_INVALID_TYPE, "tivxSetSupplementaryDataObjDesc returns VX_ERROR_INVALID_TYPE when type does not match");

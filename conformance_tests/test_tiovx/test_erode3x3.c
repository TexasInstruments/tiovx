...
static void sequential_erode3x3_check(CT_Image src, CT_Image dst, vx_border_t border)
{
    ...
    ASSERT_NO_FAILURE(virt_ref = erode3x3_create_reference_image(src, border));

    ASSERT_NO_FAILURE(dst_ref = erode3x3_create_reference_image(virt_ref, border));
...
#define PARAMETERS \
    CT_GENERATE_PARAMETERS("randomInput", ADD_VX_BORDERS_REQUIRE_UNDEFINED_ONLY, ADD_SIZE_18x18, ARG, erode3x3_generate_random, NULL), \
...
)
{
    ...
    ASSERT_NO_FAILURE(sequential_erode3x3_check(src, dst, border));
...
TEST_WITH_ARG(tivxErode3x3, testValidRegion, Arg,
    PARAMETERS
)
{
    vx_context context = context_->vx_context_;
    ...
    node1 = vxErode3x3Node(graph, src_image, virt);
    ASSERT_VX_OBJECT(node1, VX_TYPE_NODE);

    VX_CALL(vxSetNodeAttribute(node1, VX_NODE_BORDER, &border, sizeof(border)));

    node2 = vxErode3x3Node(graph, virt, dst_image);
...
TESTCASE_TESTS(tivxErode3x3, testGraphProcessing, testValidRegion, negativeTestBorderMode)

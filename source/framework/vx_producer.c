...
static int32_t send_reference_info(vx_producer producer, client_context* client)
{
    ...
    {
        VX_PRINT(
            VX_ZONE_ERROR, "PRODUCER: Cannot transmitt TIVX object data, insufficient socket message size %s", "\n");
        return status;
    }
        ...
        switch(producer->graph_state)
        {
            ...
            case VX_PROD_STATE_RUN:
            {
                ...
                for (vx_uint32 current_ref_num = 0; current_ref_num < num_ready; current_ref_num++)
                {
                    ...
                    producer->enqueuecount = producer->enqueuecount - 1;
                        ...
                        else
                        {
                            ...
                            for (uint32_t i = 0U; i < VX_GW_NUM_CLIENTS; i++)
                            {
                                ...
                                if (producer->consumers_list[i].state == PROD_STATE_CLI_CONNECTED)
                                {
                                    ...
                                    producer->consumers_list[i].consumer_id = i;
                                    producer->consumers_list[i].m_receiver_ctx.m_port_map = producer->ippc_port[i+1];
                            ...
                            else
                            {
                                ...
                                if (producer->enqueuecount > 0)
                                {
                                    ...
                                    vx_int32 sent_messages =
                                        send_id_message_consumers(producer, NULL, &producer->refs[buffer_id]);
#elif SOCKET_ENABLED
                                    vx_int32 sent_messages =
                                ...
                                else
                                {
                                    ...
                                    VX_PRINT(
                                        ...
                                        "directly, producer enqueue count %d, amount of last frames dropped %d\n",
                                        producer->name,
                                        producer->enqueuecount,
            ...
            case VX_PROD_STATE_WAIT:
            {
                ...
                for (waitCount = 0; waitCount < producer->numBuffers; waitCount++)
                {
                    ...
                    if (l_inGraph != NULL)
                    {
                        ...
                        producer->enqueuecount = producer->enqueuecount - num_deque_refs;
                    }
                }

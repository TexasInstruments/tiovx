...
<justification end> */
            if(obj_desc->mem_ptr.host_ptr != (uint64_t)0)
            {
                    ...
                    (dst_y < obj_desc->dst_height))
                {
                    ...
                    {
                        tivxCheckStatus(&status, ownMemBufferMap(remap_point, (uint32_t)sizeof(tivx_remap_point_t),
                            (vx_enum)VX_MEMORY_TYPE_HOST, (vx_enum)VX_WRITE_ONLY));
                    }
                    ...
                    {
                        tivxCheckStatus(&status, ownMemBufferUnmap(remap_point, (uint32_t)sizeof(tivx_remap_point_t),
                            (vx_enum)VX_MEMORY_TYPE_HOST, (vx_enum)VX_WRITE_ONLY));
                    }
